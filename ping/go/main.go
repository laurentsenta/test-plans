...
func runPing(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	...
	var (
		hostId = host.ID()
		ai     = &compat.AddrInfo{ID: hostId, Addrs: host.Addrs()}

		// the peers topic where all instances will advertise their AddrInfo.
		peersTopic = sync.NewTopic("peers", new(compat.AddrInfo))

		// initialize a slice to store the AddrInfos of all other peers in the run.
		peers = make([]*compat.AddrInfo, 0, runenv.TestInstanceCount)
	)
	...
	// in the peers slice.
	peersCh := make(chan *compat.AddrInfo)
